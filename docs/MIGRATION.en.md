...
4. **Update domain identity in code** — one script in the repo root does it all (added 2026-09-15, replacing the previous manual grep-and-edit):
   ...
   ```
   Treat the script's **current file and match counts** as authoritative: it updates the ten language pages (canonical + hreflang×12 + og:url + og:image + JSON-LD + footer signature), `sitemap.xml`, `robots.txt` (including the functional Sitemap declaration), both READMEs, the `404.html` footer, `assets/wall.js` (the anonymous-email domain `local.xxc2007.me` is an identity string only), `docs/promo/*`, and **this migration guide itself**. Always review the result with `git diff` — check `canonical` / `hreflang` / `og:url` / `sitemap.xml` in particular — then commit.
