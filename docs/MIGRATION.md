...
4. **改代码里的域名标识**（一条命令找全，全口径 8 个文件 37 行）：
   ```bash
   grep -rn "xxc2007.me" --include="*.html" --include="*.xml" --include="*.md" --include="*.js" --include="*.txt" .
   ```
   涉及：`index.html` 的 canonical / hreflang×3 / og:url / og:image / JSON-LD / 页脚签名共 8 行、`en/index.html` 同构 8 行、`sitemap.xml` 两个 URL 与交替链接共 8 行、双语 README 各 3 行、`robots.txt` 2 行（含 Sitemap 声明，功能行必改）、`404.html` 页脚签名 1 行、`assets/wall.js` 4 行（匿名邮箱域 `local.xxc2007.me`，仅作身份串不渲染，漏改不影响功能，建议顺手改齐）。改完提交。
