...
这是南昌市第十五中学的纪念页：**27 张校园实景摄影**、一段**可点击机位的时光漫游 + 中文定位图（MapLibre GL）**、一面**无需登录的匿名留言墙**，以及一整套为阅读体验打磨的动态交互——阅读进度条、章节导航高亮、数字滚动、时间线生长、可缩放平移的灯箱。
...
- 滚动临近地图区才按需加载 MapLibre，首屏零地图开销；机位数据集中在 `assets/map.js` 的 `SPOTS` 数组，一眼可改

<p align="center">
  <img src="docs/screenshot-map.png" alt="校园寻踪：3D 卫星视角下的八个机位标记" width="86%">
</p>
...
| 光尘氛围 | 全站 canvas 光尘粒子随呼吸明灭，鼠标柔和斥力；留言墙另有「暗河流沙」流场粒子 |
...
│   ├── main.js         # 主交互：进度条/scrollspy/视差/惯性滚动/粒子/灯箱
...
| 地图 | [MapLibre GL](https://maplibre.org) v5 + Esri 卫星影像 + AWS 3D 地形 |
