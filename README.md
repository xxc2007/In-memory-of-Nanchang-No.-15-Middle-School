...
这是南昌市第十五中学的纪念页：**27 张校园实景摄影**、一座**可点击机位的 3D 卫星校园地图（MapLibre GL）**、一面**无需登录的匿名留言墙**，以及一整套为阅读体验打磨的动态交互——阅读进度条、章节导航高亮、数字滚动、时间线生长、可缩放平移的灯箱。
...
- **Esri 高清卫星影像 + AWS 地形高程**，WebGL 渲染：默认 55° 俯仰斜视角，拖动旋转、右键俯仰，加载时电影式 flyTo 运镜
- **8 个照片机位标注**，点击标记镜头飞至机位并弹出照片卡片；机位数据集中在 `CAMERA_SPOTS` 数组，一眼可改
...
| 香樟叶氛围 | 首屏 canvas 飘落淡彩叶片，随视口与标签页可见性自动启停 |
